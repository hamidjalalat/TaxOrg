...
try
{
    ...
    var result = await http.PostAsJsonAsync(requestUri: " http://localhost:5000/api/CancelInvoices", invoices);

    while (result.StatusCode == HttpStatusCode.OK)
    {
        Console.WriteLine("100 row affected");
        result = await http.PostAsJsonAsync(requestUri: "http://localhost:5000/api/CancelInvoices", invoices);
    }
...
try
{
    ...
    var result = await http.PostAsJsonAsync(requestUri: " http://localhost:5000/api/EditInvoices", invoices);

    while (result.StatusCode == HttpStatusCode.OK)
    {
        Console.WriteLine("100 row affected");
        result = await http.PostAsJsonAsync(requestUri: "http://localhost:5000/api/EditInvoices", invoices);
    }
